...
import SavePost from '/src/Components/Blog/BlogSfFr/SavePost';
import { useAuth } from '/src/contexts/AuthContext';
import { db } from '/src/environments/firebase-config';
import { serverTimestamp, getDoc, doc, deleteDoc, where, getDocs, collection, query } from 'firebase/firestore';
...
const BlogPostForm = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setFeedback({
        message: "Le fichier est trop lourd. La taille maximum est de 4MB.",
        isSuccess: false,
        type: 'validationError',
      });
      return;
  ...
  const handleSubmit = async (event) => {
    ...
    let newPostData = { ...postData, postAuthor: userData?.uid || '' };

    if (!postId) {
      newPostData.postDate = serverTimestamp();
    }
    ...
    try {
      ...
      const success = await SavePost(newPostData, postId);
